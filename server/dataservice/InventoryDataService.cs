...
namespace Momantza.Services
{
    ...
    public class InventoryDataService : BaseDataService<InventoryItem>, IInventoryDataService
    {
        ...
        public async Task<InventoryItem> CreateAsync(InventoryItem item)
        {
            ...
            var sql = @"
        INSERT INTO inventory_items (id, name, quantity, price, organizationid,booking_id)
        VALUES (@id, @name, @quantity, @price, @organizationid, @bookingid)";
        ...
        {
            var sql = @"INSERT INTO inventory_items (id, name, quantity, price,organizationid,booking_id, created_at, updated_at) 
                VALUES (@id, @name, @quantity,@price, @organizationid,@bookingid, @created_at, @updated_at)";
        ...
        protected override (string sql, Dictionary<string, object?> parameters, List<string> jsonFields) GenerateUpdateSql(InventoryItem entity)
        {
            var sql = @"UPDATE inventory_items 
                SET name = @name, quantity = @quantity, price = @price,  updated_at = @updated_at 
                WHERE id = @id";
            var parameters = new Dictionary<string, object?>
            {
                ["@id"] = entity.Id,
