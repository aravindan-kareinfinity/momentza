...
export const getApiBaseUrl = (): string => {
  ...
  // 1️⃣ Dev localhost subdomains (appointza.localhost)
  ...
  // 2️⃣ Plain localhost
  if (hostname === 'localhost' || hostname === '127.0.0.1') {
    return `${protocol}//localhost:${BACKEND_PORT}`;
  }

  // 3️⃣ Production subdomains (storesoft.momentza.com)
  if (hostname.split('.').length >= 3) {
    return `${protocol}//${hostname}`;
  }

  // 4️⃣ Root production domain (momentza.com)
  return '';
};
