...
export class ApiServicesService implements IServicesService {
  ...
  async updateService(
    ...
    return apiClient.post<ServiceItem>(
      `/api/services/setting/update/${id}`,
      updates
    );
  }

  async updateSettingsService(
    id: string,
    updates: Partial<ServiceItem>
  ): Promise<ServiceItem> {
<<<<<<< HEAD
...
>>>>>>> d23f179 (Resolve merge conflicts and apply settings screen changes)
  ...
  async getServiceByBookingId(bookingId: string): Promise<ServiceItem[]> {
    return apiClient.get<ServiceItem[]>(
      `/api/Services/bookings/${bookingId}/services`
    );
  }
}
